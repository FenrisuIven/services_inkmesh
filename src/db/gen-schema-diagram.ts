...
async function generatePlantUML() {
  ...
  for (const [exportName, { table, file }] of Object.entries(tables)) {
    ...
    for (const columnKey of Object.keys(config.columns)) {
      ...
      const type = (col as any).getSQLType();
    ...
    for (const fk of config.foreignKeys) {
      ...
      const foreignExportName = Object.entries(tables).find(([_, { table: t }]) => getTableConfig(t).name === foreignTableName)?.[0] || foreignTableName;
      
      const colNames = ref.columns.map(c => c.name).join(", ");
      relations.push(`${exportName} }o--|| ${foreignExportName} : "${colNames}"`);
    }
  }
